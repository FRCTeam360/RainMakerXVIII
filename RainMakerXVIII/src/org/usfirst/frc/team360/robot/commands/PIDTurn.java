...
public class PIDTurn extends Command {
    ...
    public PIDTurn(double direction) { //direction is called as 270
...
//    	}
    ...
    protected void execute() {
    	...
    	pAdjustment = error * RobotMap.PIDTurnP * RobotMap.PIDTurnMultiplier;
    	iAdjustment = iAdjustment + (error * RobotMap.PIDTurnI * RobotMap.PIDTurnMultiplier);
    	dAdjustment = (error - lastError) * RobotMap.PIDTurnD * RobotMap.PIDTurnMultiplier;
    	lastError = error;
...
//      SmartDashboard.putNumber("left: ", motorSpeed + PIDAdjustment);
    	...
    	Robot.drivetrain.driveR(-(speed));
  		...
  		if(Robot.navX.getNavXAngle() < 2 + direction && Robot.navX.getNavXAngle() > direction - 2){
  			n++;
  			i++;
  		} else {
  			n = 0;
    ...
    protected boolean isFinished() {
        return (Robot.navX.getNavXAngle() < 2 + direction && Robot.navX.getNavXAngle() > direction - 2 && n > 5);// || time.get() > 3;
    	//return false;
    }
